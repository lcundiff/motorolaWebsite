(function () {
	...
	function InterviewsAdminsController($scope, $state, $window, $filter, Authentication, Notification, AdminService, UsersService, StudentService, FileService, VolunteerService, /* AutomateService, googleDriveService,*/ $http, $sce) {
		...
		function addInterviewer(student, volunteerUserID, index) {
			console.log("Current interviewerID: ",student.interviewerID[index]);
			console.log("Current interviewer: ",student.interviewer[index]);
			if (student.interviewer[index] != null) {
				Notification.error({
			...
			vm.loading = true;
			student.interviewerID[index] = volunteerUserID;
			//console.log("NEW VOLUNTEER ID HERE: ",volunteerUserID);

			VolunteerService.getVolunteer(volunteerUserID).then(function (data) {
				...
				student.interviewer[index] = volunteer.application.firstName;				
				console.log("VOLUNTEER NAME HERE: ", student.interviewer[index]);
		...
		function removeInterviewer(student, volunteerUserID, index) {
			...
			console.log("InterviewerID: ",student.interviewerID);
		...
		function onAddInterviewerError(response) {
			...
			Notification.error({
				message: response.data.message,
				title: '<i class="glyphicon glyphicon-remove"></i> There was an error adding interviewer to student.',
				delay: 6000
			});
		}
